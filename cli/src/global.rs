...
use bsvdb_chainstore::Result as ChainStoreResult;
...
pub async fn sync_piped(config: &BSVDBConfig) -> CliResult<()> {
    ...
    pub async fn stage1(mut block_iter: Pin<Box<dyn BlockHashListStream<Item=BlockHash>>>, config: &BSVDBConfig,
                        sender: Sender<Stage1Result>) -> CliResult<()> {
        let mut chain_store = FDBChainStore::new(&config.chain_store, config.get_blockchain_id()).await?;
        while let Some(block_hash) = block_iter.next().await {
            let t = chain_store.get_block_info_by_hash(&block_hash);
            let f = tokio::spawn(t);
            sender.send((f, block_hash)).await;
        }
        Ok(())
    }
    ...
    let f_stage1 = stage1(i, config.clone(), sender);
