...
async fn run_fdb_tests() {
    ...
    let (chain_store, j) = FDBChainStore::new(&config, BlockchainId::Main)
        .await
        .unwrap();
    ...
    d.remove(&tx, &root_dir)
        .await
        .expect("error removing test directory");
...
async fn check_clone_store(chain_store: &FDBChainStore) {
    ...
    let j = tokio::spawn(async move { c2.get_block_info(0).await });
...
async fn check_store(chain_store: &FDBChainStore) {
    ...
    assert_eq!(
        g_block.hash,
        BlockHeader::get_genesis(BlockchainId::Main).hash()
    );
    assert_eq!(g_block.height, 0);
    assert_eq!(g_block.size, Some(285));

    // get genesis block by hash
    let g_block2 = chain_store
        .get_block_info_by_hash(BlockHeader::get_genesis(BlockchainId::Main).hash())
        .await
        .unwrap()
        .unwrap();
    ...
    assert_eq!(
        info1.header.hash(),
        BlockHash::from_hex("00000000839a8e6886ab5951d76f411475428afc90947ee320161bbf18eb6048")
            .unwrap()
    );
    assert_eq!(
        info1.header.prev_hash,
